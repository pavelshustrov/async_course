...
const (
	...
	CreateNewTask = `insert into tasks(public_id, description, status, assignee_id) values ($1, $2, $3, $4)`
...
func (r *repo) Create(ctx context.Context, task *services.Task) (*services.Task, error) {
	...
	_, err = tx.Exec(ctx, CreateNewTask, task.PublicID, task.Description, task.Status, task.AssigneeID)
	if err != nil {
		return nil, err
	}
	...
	_, err = tx.Exec(ctx, AddNewJob, "task.created", createdNewTaskEventPayload)
	if err != nil {
		return nil, err
	}
...
func (r *repo) Close(ctx context.Context, task *services.Task) error {
	...
	_, err = tx.Exec(ctx, AddNewJob, "task.closed", createdNewTaskEventPayload)
	if err != nil {
		return err
	}
...
func (r *repo) Reassign(ctx context.Context) error {
	...
	for _, openTaskPublicID := range openTaskPublicIDs {
		...
		_, err = tx.Exec(ctx, AddNewJob, "task.reassigned", createdNewTaskEventPayload)
		if err != nil {
			return err
		}
